...
4. **レビュー（Codex → 人）** → `template/CLAUDE.md` の「Codexによるレビューの実行方法」を以下の読み替えで適用する。バックグラウンド実行、`-o`（result.md）と full.log の分離、`resume` による逐次再レビュー、重要度「高・中」ゼロ収束→人レビューの順序はそのまま従う
   - 保存先: `docs/issues/{案件フォルダ}/reviews/`（full.log は `.gitignore` 済み）
